...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="styles.css?v=20260628-14">
  </head>
  <body>
    <main class="app-shell">
      <section class="viewer-panel" aria-label="360 panorama viewer">
        ...
        <button id="fullscreenReset" class="fullscreen-reset hidden" type="button" title="Refresh view" aria-label="Refresh view">&#8634;</button>
        <div id="fullscreenZoom" class="fullscreen-zoom" aria-label="Fullscreen zoom"><span>Zoom</span><input id="fullscreenFovRange" type="range" min="35" max="95" value="70"></div>
      </section>
    ...
    <script src="app.js?v=20260628-14"></script>
  </body>
</html>
`;
  }
  ...
  async function openTemporaryPreview() {
    ...
    const [viewerScript, viewerStyles] = await Promise.all([
      fetch("../3dviewdesigns/as005/room/view1/app.js?v=20260628-14").then((response) => response.text()),
      fetch("../3dviewdesigns/as005/room/view1/styles.css?v=20260628-14").then((response) => response.text())
    ]);
